		...
		<div class="row">
			<div class="col-12">
				<nav class="main-nav">
					...
					<!-- ***** Menu Start ***** -->
					<ul class="nav">
						<li><a href="${cpath}/" class="pink" id="navIntro">Intro</a></li>
						<%-- 비회원은 내 음악 클릭 시 로그인 모달창 --%>
						<c:if test="${member==null}">
							<li><a id="navMypage" data-bs-toggle="modal"
								data-bs-target="#loginModal">내 음악</a></li>
						</c:if>
						<%-- 로그인한 회원은 내 음악 페이지로 이동 --%>
						<c:if test="${member!=null}">
							<li><a id="navMypage" href="${cpath}/mypage">내 음악</a></li>
						</c:if>

						<%-- 비회원은 AI추천 클릭 시 로그인 모달창 --%>
						<c:if test="${member==null}">
							<li><a id="navAi" data-bs-toggle="modal"
								data-bs-target="#loginModal">AI추천</a></li>
						</c:if>
						<%-- 로그인한 회원은 AI추천 페이지로 이동 --%>
						<c:if test="${member!=null}">
							<li><a id="navAi" href="${cpath}/join">AI추천</a></li>
						</c:if>
						...
						<c:if test="${member != null}">
							<div class="dropdown">
								<button type="button" id="profileDropdown" class="btn dropdown-toggle"
									data-bs-toggle="dropdown"><span id="memberName">${member.name}님</span></button>
								<ul class="dropdown-menu dropdown-menu-dark">
									<li><a class="dropdown-item" id="btnUpdate" href="${cpath}/update">프로필
											수정</a></li>
									<li><a class="dropdown-item" id="btnMypage" href="${cpath}/mypage">나의
											뮤직</a></li>
									<li>
